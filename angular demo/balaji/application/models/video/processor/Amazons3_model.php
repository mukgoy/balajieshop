...
class Amazons3_Model extends CI_Model{
	...
	public function __construct() {
		$accessKey = 'AKIAJBZ2IDLSZQAZECKA';
		$secretKey = '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';
			
		self::setAuth($accessKey, $secretKey);
	}

	public function uploadAWS($sourceFile, $destinationFile="", $bucket = ''){
		if($bucket == ''){ $bucket = $GLOBALS['s3_bucket']; }
		if($destinationFile==""){
			$destinationFile = $sourceFile;
		}
		$this->putBucket($bucket);
		if(file_exists($sourceFile)){
			if($this->putObjectFile($sourceFile, $bucket , $destinationFile)){
	...
	public function deleteAWS($uri = '', $bucket = ''){
		if($bucket == ''){ $bucket = $GLOBALS['s3_bucket']; }
		if($uri == ''){
			return;
		}
		$this->deleteObject($bucket, $uri);
	}
	...
	* @return boolean
	*/
	public function putBucket($bucket, $acl = self::ACL_PUBLIC_READ_WRITE) {
	...
	* @return boolean
	*/
	public static function putObjectFile($file, $bucket, $uri, $acl = self::ACL_PUBLIC_READ_WRITE, $metaHeaders = array(), $contentType = null) {
	...
	* @return mixed
	*/
	public static function deleteObject($bucket = 'videowhizz', $uri = '') {
